"""Unit tests for dev manifest filtering (no network)."""
...
from src.services.manifest_filter import (
    ...
    limit_manifest_one_per_category,
)
def test_limit_one_per_category() -> None:
    manifest = [
        {"subsection_key": "a", "display_title": "Engagement Horizon"},
        {"subsection_key": "b", "display_title": "Import / Export"},
        {"subsection_key": "c", "display_title": "Midyear Metrics"},
        {"subsection_key": "d", "display_title": "Artist Spectrum"},
    ]
    out = limit_manifest_one_per_category(manifest)
    assert len(out) == 3
    titles = [r["display_title"] for r in out]
    assert "Engagement Horizon" in titles
    assert "Import / Export" in titles
    assert "Midyear Metrics" in titles
    assert "Artist Spectrum" not in titles
...
def test_dev_profile_keeps_first_of_duplicate_category() -> None:
    manifest = [
        {"subsection_key": "a", "display_title": "Engagement Horizon"},
        {"subsection_key": "b", "display_title": "Superfans and Gaming"},
        {"subsection_key": "c", "display_title": "Import / Export"},
    ]
    out = limit_manifest_one_per_category(manifest)
    assert len(out) == 2
    assert out[0]["display_title"] == "Engagement Horizon"
    assert out[1]["display_title"] == "Import / Export"
